...
public class StreamHTTPClient {
    ...
    @NotNull
    private final String sdkVersion = readSdkVersion();
    ...
    private final ObjectMapper objectMapper =
            JsonMapper.builder()
                    ...
                    .addModule(new JavaTimeModule().addDeserializer(Instant.class, new JsonDeserializer<Instant>() {
                        @Override
                        public Instant deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
                            if (p.hasToken(com.fasterxml.jackson.core.JsonToken.VALUE_STRING)) {
                                String stringValue = p.getText();
                                try {
                                    // Try parsing as ISO-8601/RFC-3339 string
                                    return Instant.parse(stringValue);
                                } catch (Exception e) {
                                    throw ctxt.weirdStringException(stringValue, Instant.class,
                                            "Unable to parse string as Instant: " + stringValue);
                                }
                            } else if (p.hasToken(com.fasterxml.jackson.core.JsonToken.VALUE_NUMBER_INT)) {
                                long nanoseconds = p.getLongValue();
                                return Instant.ofEpochSecond(0, nanoseconds);
                            } else {
                                throw ctxt.wrongTokenException(p, Instant.class,
                                        com.fasterxml.jackson.core.JsonToken.VALUE_NUMBER_INT,
                                        "Expected string or number for Instant");
                            }
                        }
                    }))
                    .build();
    ...
    @NotNull
    private String baseUrl = API_DEFAULT_URL;
    @NotNull
