...
import io.getstream.models.framework.User;
...
public class DocTest extends BasicTest {
    ...
    public void getStarted() throws Exception {
        ...
        User.createToken("john", Instant.now().plus(24, ChronoUnit.HOURS), Instant.now());
    ...
    public void authentication() throws Exception {
        ...
        client.common().updateUsers(UpdateUsersRequest.builder()
                .users(Map.of("todeactivate", UserRequest.builder().id("todeactivate").build()))
                .build()).execute();

        // deactivate a user
        client.common().deactivateUser("todeactivate", DeactivateUserRequest.builder().build()).execute();

        client.common().reactivateUser("todeactivate", ReactivateUserRequest.builder().build()).execute();

        var taskId = client
                .common()
                .deactivateUsers(
                        DeactivateUsersRequest.builder().userIds(List.of("todeactivate")).build())
                .execute().getData().getTaskID();
        ...
        var response =
                client
                        .common()
                        .exportUsers(
                                ExportUsersRequest.builder().userIds(List.of("john")).build())
                        .execute();
        var taskID = response.getData().getTaskID();

        // get information about the task
        var taskStatus = client.common().getTask(taskID).execute();
        ...
        while (true) {
            ...
            if (status.equals("failed") || status.equals("error")) {
                throw new StreamException(
                        String.format(
                                "Failed to delete user(task_id: %s): %s",
                                response.getData().getTaskID(), status),
                        (Throwable) null);
            }
        ...
        client
                .common()
                .restoreUsers(
                        RestoreUsersRequest.builder().userIds(List.of("todelete")).build())
                .execute();
        ...
        // call tokens
        var callToken =
                User.createCallToken(new CallTokenClaims("user-id", "admin", List.of("default:call1", "livestream:call2"), 3600));
    }
    ...
    public void calls() throws StreamException {
                        ...
                        CreateCallTypeRequest.builder()
                                ...
                                .grants(
                                        Map.of(
                                                ...
                                                "user",
                                                List.of(OwnCapability.SEND_AUDIO.toString(), OwnCapability.SEND_VIDEO.toString())))
                                .build())
                .execute();
        ...
        call.videoPin(PinRequest.builder().userID("user-id-to-pin").sessionID("session-id").build());
        call.videoUnpin(
                UnpinRequest.builder().userID("user-id-to-unpin").sessionID("session-id").build());
    ...
    public void multiTenant() throws StreamException {
        ...
        client
                .common()
                .updateApp(UpdateAppRequest.builder().multiTenantEnabled(true).build())
                .execute();
        ...
        client
                .common()
                .queryUsers(QueryUsersPayload.builder().filterConditions(new HashMap<>() {
                    {
                        put("teams", null);
                    }
                }).build())
        ...
        client
                ...
                .queryCalls(
                        QueryCallsRequest.builder()
                                .filterConditions(Map.of("cid", callCID, "team", new HashMap<>() {
                                    {
                                        put("$eq", null);
                                    }
                                }))
